...
import type { Database } from './sql-adapter'
import { existsSync, mkdirSync } from 'node:fs'
import { homedir } from 'node:os'
import { join } from 'pathe'
import { createDatabase, saveDatabase } from './sql-adapter'
...
export class MemoryTree {
  ...
  async getAllNodes(options: {
    ...
    return results[0].values.map((row: any[]) => this.rowToNode(results[0].columns, row))
  }
