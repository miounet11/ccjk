...
import type { Message, IPubSub, MessageHandler } from '../types.js';
...
export class PubSub implements IPubSub {
  ...
  async getChannels(): Promise<string[]> {
    ...
    const channels = await this.subscriber.channels();
  ...
  async getSubscribers(channel: string): Promise<number> {
    ...
    const info = await this.redis.pubsub('NUMSUB', channelKey);
    return info[1] || 0;
  }
  ...
  async getMessageHistory(channel: string, count = 10): Promise<Array<{
    ...
    const results = await this.redis.zrange(historyKey, -count, -1, {
      REV: true,
    });
