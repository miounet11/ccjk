...
import type { Message, IMessageQueue, MessagePriority } from '../types.js';
...
export class MessageQueue implements IMessageQueue {
  ...
  async dequeue(queue: string): Promise<Message | null> {
    ...
    try {
      ...
      const priorityKey = this.getPriorityKey(queue, message.priority);
      await this.redis.pfrem(priorityKey, message.id);
  ...
  async peek(queue: string, count = 10): Promise<Message[]> {
    ...
    // Get top N messages without removing them
    const results = await this.redis.zrange(queueKey, -count, -1, {
      REV: true,
    });
      ...
      for (const messageData of messages) {
        try {
          ...
          if (message.id === messageId) {
            ...
            // Remove from priority index
            const priorityKey = this.getPriorityKey(queue, message.priority);
            pipeline.pfrem(priorityKey, messageId);
  ...
  async getDeadLetterMessages(queue: string, count = 10): Promise<Array<{
    ...
    const results = await this.redis.zrange(deadLetterQueue, -count, -1, {
      REV: true,
    });
