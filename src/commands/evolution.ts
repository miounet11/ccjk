...
interface EvolutionClient {
  hello: (name: string, version: string, capabilities?: string[]) => Promise<unknown>
  fetch: (options?: { minGDI?: number, limit?: number, signature?: string, geneId?: string }) => Promise<Gene[]>
  stats: () => Promise<{
    totalGenesInPool: number
...
export async function handleEvolutionCommand(
  ...
  try {
    const client = new A2AClient(getCloudBaseUrl('MAIN')) as unknown as EvolutionClient
 ...
 */
async function showTopCapabilities(client: EvolutionClient, options: any): Promise<void> {
 ...
 * Search for solutions
 */
async function searchSolutions(client: EvolutionClient, query: string, options: any): Promise<void> {
 ...
 * Show gene details
 */
async function showGeneDetails(client: EvolutionClient, geneId: string): Promise<void> {
  // Search by ID prefix
 ...
 * Show statistics
 */
async function showStats(client: EvolutionClient): Promise<void> {
