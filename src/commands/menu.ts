...
const ITEMS = [
  ...
  { key: '2', label: '配置 MCP 服务', run: () => mcpCommand() },
  { key: '3', label: '体检（doctor）', run: () => doctorCommand() },
  { key: '4', label: '安装 Git 命令模板', run: () => gitInstallCommand() },
  { key: '5', label: '检测已安装的工具', run: async () => detectCommand() },
];
...
export async function menuCommand(): Promise<void> {
  ...
  const { choice } = await inquirer.prompt<{ choice: string }>([{
    ...
    name: 'choice',
    message: '\n选择',
    validate: (v: string) => /^[1-5q]$/.test(v.trim()) || '输入 1-5 或 q',
  }]);
