...
afterEach(() => {
  if (existsSync(mockSettingsFile))
    rmSync(mockSettingsFile);
  if (existsSync(mockAgentsDir))
    rmSync(mockAgentsDir, { recursive: true });
  if (existsSync(mockSkillsDir))
    rmSync(mockSkillsDir, { recursive: true });
});
