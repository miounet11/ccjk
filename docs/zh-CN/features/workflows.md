...
CCJK 通过 `WORKFLOW_CONFIG_BASE` 预置多套工作流，并在初始化或更新时自动导入：
...
- `ccjk init` 默认导入全部工作流，用户可通过 `--workflows` 选择性安装。
- `ccjk update` 在模板更新后会再次执行工作流导入，确保内容同步。
- 工作流文件会自动安装到 Claude Code/Codex 的 `prompts/workflows/` 目录。

## 代理自动安装

- 对于要求代理的工作流（如 `featPlanUx`），CCJK 会同步复制 `agents/planner.md`、`agents/ui-ux-designer.md`。
- 支持根据 `autoInstallAgents` 字段自动处理。

## 命令格式

CCJK 工作流在不同工具中使用不同的命令前缀：
...
> 💡 **提示**：Codex 使用 `/prompts:` 前缀访问所有工作流命令，而 Claude Code 使用 `/ccjk:` 前缀或直接 `/` 前缀。

## 使用建议

- 首次使用工作流时，可让 AI 输出任务进度文档，方便后续在新对话继续
  - Claude Code：`/ccjk:workflow <任务描述>`
