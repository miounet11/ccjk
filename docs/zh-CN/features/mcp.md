...
CCJK 内置常用 MCP（Model Context Protocol）服务配置，支持一键安装和管理多个 MCP 服务，扩展 AI 助手的能力边界。
...
> 💡 **提示**：环境变量可以通过 CCJK 菜单选项 `7` 导入推荐的环境变量配置。
...
CCJK 会自动修正 Windows 路径格式，确保 MCP 服务在 Windows 上正常运行：

- 自动检测 Windows 环境
- 修正路径分隔符和转义字符
- 使用 `cmd /c npx` 格式确保命令执行

> 💡 **提示**：如果在 Windows 上遇到 MCP 连接问题，运行 `npx ccjk` 会自动修复配置格式。

### WSL 和 Termux

CCJK 完全支持 WSL 和 Termux 环境，MCP 服务在这些环境中同样可以正常工作。
...
npx ccjk
# 选择 4. 配置 MCP
# CCJK 会自动修复 Windows 路径格式
```

## 最佳实践

1. **按需安装**：只安装实际需要的 MCP 服务，减少资源消耗
2. **定期更新**：通过 `npx ccjk update` 更新 MCP 服务配置
