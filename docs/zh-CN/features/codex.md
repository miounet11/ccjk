...
[Codex](https://www.npmjs.com/package/@openai/codex) 是 OpenAI 官方的代码生成 CLI 工具。CCJK 现在支持完整的 Codex 集成，具备与 Claude Code 相同的配置便利性。

## 核心特性

CCJK 为 Codex 提供完整的自动化配置体验，核心特性包括：

- **统一工具管理**：通过 CCJK 菜单在 Claude Code 和 Codex 之间无缝切换
...
CCJK 会自动检测系统是否已安装 `@openai/codex` CLI：
...
# 或通过菜单
npx ccjk → 选择 +（检查更新）→ 选择 Codex
```

> ✅ **自动处理**：如果升级失败，CCJK 会提供详细的错误信息，帮助诊断问题。

## 配置文件管理

### 目录结构

CCJK 为 Codex 创建以下目录结构：
...
CCJK 在配置文件中添加 `managed = true` 标识，确保：

- 自动判断配置是否由 CCJK 管理
- 避免覆盖手写配置
- 智能合并 CCJK 管理的配置和用户自定义配置
...
> 📖 **切换提供商**：使用 `npx ccjk config-switch -T codex` 在多个提供商之间切换。
...
# 通过菜单配置
npx ccjk → 选择 S（切换到 Codex）→ 选择 4（配置 MCP）
```
...
CCJK 允许在 Claude Code 和 Codex 之间无缝切换，同时保留您的偏好设置和工作流配置：
...
# 交互式方式
npx ccjk → 选择 S（切换到 Codex）→ 选择 1（完整初始化）
```
...
> 💡 **说明**：虽然配置文件格式不同，但 CCJK 提供了统一的配置接口，确保两个工具的使用体验一致。
