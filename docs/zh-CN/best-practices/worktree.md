...
Git Worktree 是 Git 的一个强大功能，允许您在同一仓库中创建多个工作区（副本），每个工作区可以切换不同的分支，互不干扰，共享同一套 Git 历史。CCJK 提供的 `/git-worktree` 智能指令让 Worktree 管理变得简单直观。
...
CCJK 的 `/git-worktree` 指令提供了完整的 Worktree 管理功能：
...
CCJK 的 `/git-worktree` 指令支持自然语言，不需要记住命令细节：
...
CCJK 的 Worktree 默认创建在项目平级的 `../.ccjk/项目名/` 目录下，避免混乱：
...
2. 在每个 Worktree 中运行工作流
   - 在 solution1 中：`/ccjk:workflow 开发xxx功能`
   - 在 solution2 中：`/ccjk:workflow 开发xxx功能，要求高性能`
   - 在 solution3 中：`/ccjk:workflow 开发xxx功能，要求简洁代码`
...
# 在新工作区中，引用工作区2的经验
/ccjk:workflow 开发xxx功能，参考solution2的实现，但要避免xxx问题
```
...
# Worktree 2：优化实现
/ccjk:workflow 实现用户登录功能，使用JWT，支持刷新token

# Worktree 3：测试实现
/ccjk:workflow 实现用户登录功能，重点考虑安全性
```
...
```text
# 在新 Worktree 中继续任务
/ccjk:workflow 继续开发xxx功能，参考之前的进度文档：progress.md
```
...
4. **合并代码**
   ```bash
   # 在 Worktree 中提交
   cd ../.ccjk/project-name/feat/feature-name
   git add .
...
- [CCJK 六阶段工作流](../workflows/ccjk-workflow.md) - 配合工作流使用
