...
`ccjk update`（缩写 `ccjk u`）用于更新工作流模板、提示词和检查工具版本。这是一个轻量级的更新命令，不会修改 API 配置或已安装的 MCP 服务。

## 功能概述

`ccjk update` 命令会执行以下操作：
...
| `--code-type, -T` | `-T` | 目标代码工具类型 | `claude-code`, `codex`, `cc`, `cx` | CCJK 配置中的当前工具类型 |
...
如果需要更新这些内容，请使用 `ccjk init` 或对应的配置菜单。

## 版本检查

`ccjk update` 会自动检查工具版本：
...
建议定期运行 `ccjk update` 以获取最新工作流和改进：
...
1. **备份自定义内容**：在更新前手动备份
2. **使用 `docs-only` 模式**：在 `ccjk init` 中使用 `--config-action docs-only`
3. **查看差异**：更新后对比备份和当前文件
...
3. **使用 check-updates 命令**：运行 `npx ccjk check-updates`
...
> 💡 **建议**：
> - 首次使用或需要修改 API/MCP 配置时，使用 `ccjk init`
> - 仅需要更新工作流和模板时，使用 `ccjk update`

## 相关资源

- [ccjk init](init.md) - 完整初始化命令
- [工作流系统](../features/workflows.md) - 工作流详细介绍
- [配置管理](../features/multi-config.md) - 备份和恢复
- [check-updates](check-updates.md) - 版本检查命令

> 💡 **提示**：建议定期运行 `ccjk update` 以保持工作流模板和提示词的最新状态，特别是在有新功能发布或模板更新时。