...
Spec 工作流作为 MCP 服务的一部分，可以在 CCJK 初始化时选择安装：
...
Spec 工作流可以与 CCJK 的其他工作流结合使用：

```bash
# 在 Claude Code 中
/ccjk:workflow 开发用户评论功能，使用 Spec 工作流进行需求分析
...
> 💡 **提示**：Spec 工作流是 CCJK 工作流生态的重要组成部分，建议在项目早期就引入，以获得最大的收益。结合 CCJK 的其他工作流和工具，可以构建完整的开发流程。