...
このガイドは、環境チェックから検証までの完全なプロセスをカバーし、CCJKを迅速に開始するのに役立ちます。初めて使用する場合でも、新しいデバイスに迅速にデプロイしたい場合でも、このガイドに従ってセットアップを完了できます。

> 💡 **ヒント**: CCJKはインストール不要で、`npx ccjk`を直接実行できます。このガイドは主に環境設定と使用フローについて説明しています。
...
> 💡 **ヒント**: WSL（Windows Subsystem for Linux）を使用している場合、CCJKは自動的に環境を検出し、対応するインストールプロンプトを提供します。
...
CCJKは2つの使用方法を提供します：**対話式使用**（初心者向け）と**コマンドライン直接使用**（自動化とCI/CD向け）。

> 💡 **ヒント**: CCJKはインストール不要で、`npx ccjk`コマンドを直接実行できます。

### 方法1: 対話式使用（初心者におすすめ）

CCJKは、グラフィカルインターフェースですべての設定を完了できる、使いやすい対話式メニューを提供します。
...
初回実行時、CCJKはウェルカム画面を表示し、使用したいインターフェース言語を尋ねます：
...
機能を選択してください:
  ...
  ------------ CCJK ------------
  0. 表示言語の変更 / Select display language - CCJKインターフェース言語の変更
...
`1`を選択して完全初期化を実行すると、CCJKは以下のステップでガイドします：
...
インストールを選択すると、CCJKはインストール方法の選択を促します：
...
> ✅ **自動処理**：
> - CCJKは自動的にプラットフォームを検出し、最適なインストール方法を推奨します
> - Claude Codeが既にインストールされている場合、CCJKはバージョンを検出し、最新バージョンに自動アップグレードできます
...
CCJKはAPIプロバイダープリセットをサポートしており、設定を5+パラメータから2つに簡素化できます：
...
CCJKは複数のAPIを設定することをサポートしており、異なるシナリオで簡単に切り替えできます：
...
> 📖 **完全なパラメータリスト**: 詳細なパラメータ説明については、[CLIコマンド - ccjk init](../cli/init.md)の章を参照してください。

## Codexサポート

CCJKは完全なCodexサポートを提供し、同じツールでClaude CodeとCodexの両方の環境を管理できます。
...
# 方法2: 対話式メニュー経由
npx ccjk → S（ツール切り替え）を選択 → 1（完全初期化）を選択
```
...
CCJKは、Windows、macOS、Linux、WSL、Termuxを含むクロスプラットフォーム操作を完全にサポートしています。
...
> ⚠️ **注意**: WindowsでMCP接続の問題が発生した場合、`npx ccjk`を実行すると設定形式が自動的に修正されます。
...
> 📱 **ヒント**: Termuxでは、CCJKは特別なパス構造を自動認識し、依存関係を正しくインストールします。
...
**Claude Code:**
```
/ccjk:workflow  # 6段階開発ワークフロー
/ccjk:feat      # 機能開発ワークフロー
...
> 
> 💡 **ヒント**: Codexは`/prompts:`プレフィックスを使用し、Claude Codeは`/ccjk:`または直接`/`プレフィックスを使用します。
...
> 🔧 **トラブルシューティング**: サービスが接続されていない場合、`npx ccjk` → `4`を実行してMCPサービスを再設定してください。
...
> 📖 **注意**: `ccjk update`は、デフォルトで既存のAPI設定とMCP設定を保持し、ワークフローテンプレートとドキュメントのみを更新します。
...
ファイルが存在しない場合、`npx ccjk update`を実行して再インポートしてください。

### Q: MCPサービスの接続に失敗しましたか？

**A**: 
1. MCPサービス設定を確認: `npx ccjk` → `4`
...
**A**: 設定切り替えコマンドを使用：
```bash
npx ccjk config-switch --list  # すべての設定を一覧表示
npx ccjk cs provider-name      # 指定された設定に切り替え
```

### Q: 設定ファイルはどこに保存されますか？

**A**: 
- **CCJK設定**: `~/.ufomiao/ccjk/config.toml`
...
1. 🎯 [機能](../features/)を探索して、CCJKの完全な機能を理解する
